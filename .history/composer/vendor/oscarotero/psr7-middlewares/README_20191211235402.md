...
$dispatcher = $relay->newInstance([
    ...
    Middleware::digestAuthentication(['usuario' => 'passwordword']),
...
$middlewares = [
    ...
    Middleware::BasicAuthentication([
        'usuario1' => 'passwordword1',
        'usuario2' => 'passwordword2'
    ]),

    //Map the key used by this middleware
    Middleware::attributeMapper([
        Middleware\BasicAuthentication::KEY => 'auth:usuario'
    ]),

    function ($request, $response, $next) {
        //We can get the usuario as usual
        $usuario = BasicAuthentication::getusuario($request);

        //But also using the "auth:usuario" attribute name.
        assert($usuario === $request->getAttribute('auth:usuario'));
...
$middlewares = [
    ...
    function ($request, $response, $next) {
        $usuario = DigestAuthentication::getusuario($request);
